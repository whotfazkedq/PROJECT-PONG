...
var click = mouse_check_button_pressed(mb_left) && point_in_rectangle(mx, my, keys_diff[selected].x1, keys_diff[selected].y1, keys_diff[selected].x2, keys_diff[selected].y2);
...
if (confirm || click) {
	switch (selected) {
		...
		case 3: room_goto(CONNECT_RM_USERNAME); break;
	}
}